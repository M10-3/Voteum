...
class electorC{
    ...
    function getElector() {
        ...
        //$users = [];
        try {
            $query = $db->prepare('SELECT * FROM elector');
            $query->execute();
...
class CandidatC{
    function addCandidat($newCandidat){
        ...
        try {
            ...
            $query = $db->prepare(
                'INSERT INTO candidat (username,full_name,email,dob,adresse,biography,program,political_party,image,password) 
                    VALUES (:username,:full_name,:email,:dob,:adresse,:biography,:program,:political_party,:image,:password)'
            );
            $query->execute([
                //'id' => $newAgence->getId(),
    ...
    function getCandidat() {
        ...
        //$users = [];
        try {
            $query = $db->prepare('SELECT * FROM candidat');
            $query->execute();
            $result=$query->fetchAll();
            return $result;
        } catch (PDOException $e) {
...
class VoteC {
    ...
    public function addVote($newVote) {
        ...
        try {
            $query = $db->prepare(
                'INSERT INTO votes (elector_id, candidate_id, election_id, voted_at) 
                VALUES (:elector_id, :candidate_id, :election_id, :voted_at)'
            );
            $query->execute([
                'elector_id' => $newVote->getId_elector(),
                'candidate_id' => $newVote->getId_candidat(),
                'election_id' => $newVote->getId_election(),
    ...
    public function updateVote($voteId, $newVote) {
        ...
        try {
            $query = $db->prepare(
                'UPDATE votes SET elector_id = :elector_id, candidate_id = :candidate_id, election_id = :election_id 
                WHERE id = :id'
            );
            $query->execute([
                'elector_id' => $newVote->getId_elector(),
    ...
    public function countVotesForElection($electionId) {
        $db = config::getConnexion();
        try {
            $query = $db->prepare('SELECT COUNT(*) FROM votes WHERE election_id = :election_id');
            $query->execute(['election_id' => $electionId]);
            $result = $query->fetchColumn();
            return $result;
        } catch (PDOException $e) {
...
class ElectionC {
    ...
    public function addElection($newElection) {
        ...
        try {
            $query = $db->prepare(
                'INSERT INTO election (title, image, start_date, end_date, is_active) 
                VALUES (:title, :image, :start_date, :end_date, :is_active)'
            );
            $query->execute([
                'title' => $newElection->getTitle(),
                'image' => $newElection->getImage(),
    ...
    public function updateElection($electionId, $newElection) {
        ...
        try {
            $query = $db->prepare(
                'UPDATE election SET title = :title, image = :image, description = :description, start_date = :start_date, end_date = :end_date, is_active = :is_active 
                WHERE id_election = :id_election'
            );
            $query->execute([
                'title' => $newElection->getTitle(),
                'image' => $newElection->getImage(),
                'description' => $newElection->getDescription(),
                'start_date' => $newElection->getStart_date(),
                'end_date' => $newElection->getEnd_date(),
                'is_active' => $newElection->getIs_active(),
    ...
    public function deleteElection($electionId) {
        ...
        try {
            $query = $db->prepare(
                'DELETE FROM election WHERE id_election = :id_election'
            );
            $query->execute([
                'id_election' => $electionId
            ]);
    ...
    public function countElections() {
        $db = config::getConnexion();
        try {
            $query = $db->prepare('SELECT COUNT(*) FROM election');
            $query->execute();
            $result = $query->fetchColumn();
            return $result;
        } catch (PDOException $e) {
    ...
    public function getElectionById($id_election) {
        $db = config::getConnexion();
        try {
            $query = $db->prepare('SELECT * FROM election WHERE id_election = :id_election');
            $query->execute(['id_election' => $id_election]);
            return $query->fetch();
        } catch (PDOException $e) {
            echo "Erreur lors de la récupération de l'élection : " . $e->getMessage();
        }
    }
...
class adminC{
    ...
    function updateAdminUsername($newName,$adminId){
        ...
        try{
            $query = $db->prepare(
                'UPDATE admin SET username= :username where id_admin = :id_admin'
            );
            $query = $query->execute([
                'username' => $newName,
                'id_admin' => $adminId
    ...
    function updateAdminImage($newImage,$adminId){
        ...
        try{
            $query = $db->prepare(
                'UPDATE admin SET image= :image where id_admin = :id_admin'
            );
            $query = $query->execute([
                'image' => $newImage,
                'id_admin' => $adminId
    ...
    function updateAdminPassword($newPassword,$adminId){
        ...
        try{
            $query = $db->prepare(
                'UPDATE admin SET password= :password where id_admin = :id_admin'
            );
            $query = $query->execute([
                'password' => $newPassword,
                'id_admin' => $adminId
